...
interface BoardSettingsPanelProps {
  isOpen: boolean;
  onClose: () => void;
  board: BoardDetailed | null; // Yönetilecek panonun tam verisi
}

/**
 * Pano adını, üyelerini (ekle/çıkar/rol) ve panoyu silme işlemlerini
 * yöneten sağdan açılan panel.
 */
const BoardSettingsPanel: React.FC<BoardSettingsPanelProps> = ({ isOpen, onClose, board }) => {
  ...
  const [memberEmail, setMemberEmail] = useState(''); // Eklenecek üyenin e-postası
  ...
  const updateNameMutation = useMutation({
    ...
    onSuccess: (updatedBoard) => {
      // Pano detay sayfasının başlığını anında güncelle
      queryClient.setQueryData(['boardDetails', board!.id], updatedBoard);
      // Pano listesi sayfasını geçersiz kıl (bir sonraki ziyarette güncellensin)
  ...
  const deleteBoardMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['userBoards'] }); // Listeyi yenile
      navigate('/boards'); // Pano listesine yönlendir
      onClose();
    },
    onError: (err) => toast.error(getErrorMessage(err, "Hata")),
  });
  ...
  const addMemberMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['boardDetails', board!.id] }); // Üye listesini yenile
      setMemberEmail('');
    },
    onError: (err) => toast.error(getErrorMessage(err, "Hata")),
  });
  ...
  if (!board) return null; // Pano verisi henüz gelmemişse (veya admin değilse)

  const isBoardCreator = board.createdBy?.id === user?.id;

  // --- TEMA GÜNCELLEMESİ (Manuel Tailwind) ---
